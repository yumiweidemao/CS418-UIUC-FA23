...
window.addEventListener('load', async (event) => {
    ...
    let vs = await fetch('mp-gpu-jitter-vertex.glsl').then(res => res.text())
    let fs = await fetch('mp-gpu-jitter-fragment.glsl').then(res => res.text())
    window.program = compileShader(vs,fs)
    let data = await fetch('mp-gpu-jitter.json').then(r=>r.json())
