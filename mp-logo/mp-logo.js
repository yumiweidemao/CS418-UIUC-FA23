...
window.addEventListener('load', async (event) => {
    ...
    let vs = await fetch('mp-logo-vertex.glsl').then(res => res.text())
    let fs = await fetch('mp-logo-fragment.glsl').then(res => res.text())
    window.program = compileShader(vs,fs)
    let data = await fetch('mp-logo.json').then(r=>r.json())
